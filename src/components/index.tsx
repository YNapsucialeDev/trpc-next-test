...
export const CardHeader: NextPage<CardHeaderProps> = ({ title, listLength, clearAllFn }) => {
    return (
        <div className='flex flex-row items-center justify-between p-3 border-b border-slate-200'>
            <div className='flex flex-row items-center justify-between'>
                <h1 className='text-base font-medium tracking-wide text-gray-900 mt-2'>{title}</h1>
                <span className='h-5 w-5 bg-blue-200 text-blue-600 flex items-center justify-center rounded-full text-xs'>
                    {listLength}
                </span>
            </div>
            <button className='text-sm font-medium text-gray-600 underline' type='button' onClick={clearAllFn}>Clear all</button>
        </div>
    )
}
...
const ListItemComponent: NextPage<ListItemProps> = ({ item, onUpdate }) => {
    return (
        <div className='h-12 border-b flex items-center justify-start px-3'>
            <input type="checkbox" className='w-4 h-4 border-gray-300 rounded mr-4' defaultChecked={item.checked as boolean} onChange={() => onUpdate?.(item)} />
...
export const CardForm: NextPage<CardFormProps> = ({ value, onChange, submit }) => {
    return (
        <div className='bg-white w-5/6 md:w-4/6 lg:w-3/6 xl:w-2/6 rounded-lg drop-shadow-md mt-4'>
            <div className='relative'>
                <input type="text" className='w-full py-4 pl-3 pr-16 text-sm rounded-lg'
                    placeholder='Nombre'
                    onChange={onChange}
                    value={value} />
                <button onClick={submit} className='absolute p-2 text-white -translate-y-1/2 bg-blue-600 rounded-full top-1/2 right-4'>
                    <svg
                        className="w-4 h-4"
